...
class SegmentationHead(nn.Sequential):
    def __init__(self, in_channels, out_channels, kernel_size=3, activation=None, upsampling=1, dropout_rate=0):
...
class CSDecoder(UnetDecoder):
    def __init__(
        ...
        block_head = [
            SegmentationHead(in_channels=in_ch, out_channels=1, activation='sigmoid', kernel_size=3, dropout_rate=dropout_rate) for in_ch in
            decoder_channels[0:]
        ]
...
class ChangeModel_CADN(torch.nn.Module):
    def __init__(
            self,
