...
class ModelCheckpoint(Callback):
    ...
    def save_checkpoint_recover(self, path, epoch, logs):
        state = {}
        state['state_dict'] = self.model.state_dict()
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if self.save_last:
            self.save_checkpoint_recover(self.last_template, epoch, logs)
...
class Scheduler(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        if self.freq == 'epoch':
            if self.sc_name=='ReduceLROnPlateau':

                score = logs[self.monitor]
                print("===========================")
                print('ReduceLROnPlateau  ', score)
                print("===========================")
