    ...
    def __call__(self, y_pred, y_true):
        ...
        if isinstance(y_pred, (list, tuple)):
            if self.weights != None:
                assert len(self.weights)==len(y_pred)
            for i, pre in enumerate(y_pred):
                if self.weights == None:
                    weight = 1.0 / (len(y_pred))
                else:
                    weight = self.weights[i]
                true = nn.functional.interpolate(y_true,
                                                  scale_factor=pre.size()[2] / y_true.size()[2],
                                                  mode='nearest')

                loss += (2 * self.bce(pre, true) + self.dice(pre, true))*weight
            return loss
        else:
