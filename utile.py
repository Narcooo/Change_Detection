...
def get_config(modelpath):
    return os.path.join(os.path.dirname(os.path.dirname(modelpath)), 'config.yml')


def get_model_config(model_path):
    config_path = []
    for index, mpath in enumerate(model_path):
        mpath = kep_path(mpath)
        cpath = get_config(mpath)
        model_path[index] = mpath
        config_path.append(cpath)
    return config_path, model_path

            
class Bigdata_change(Dataset):
    def __init__(
            self,
            img_A_path: str,
            img_B_path: str,
        ...
        self.dataset_B=cv2.imread(self.img_B_path, -1)       #打开文件
    ...
    def __getitem__(self, i):
        ...
        sample['image'] = self.tfms(np.ascontiguousarray(sample['image']).astype("float32")/255.0).float().unsqueeze(0)
